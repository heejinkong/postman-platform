    ...
    <TreeItem
      ...
      label={
        ...
        >
          ...
          {request.method === 'GET' ? (
            <Box>
              <Typography style={{ color: '#2E7D32', fontWeight:500}} fontSize="inherit">GET</Typography> 
            </Box>            
          ) : request.method === 'POST' ? (
            <Typography style={{ color: '#E65100', fontWeight:500}} fontSize="inherit">POST</Typography>
          ) : request.method === 'PUT' ? (
            <Typography style={{ color: '#673AB7', fontWeight:500 }} fontSize="inherit">PUT</Typography>
          ) : request.method === 'DELETE' ? (
            <Typography style={{ color: '#795548', fontWeight:500 }} fontSize="inherit">DELETE</Typography>
          ) : null}

          {/* request title 표시 */}
          <Box sx={{ flexGrow: 1, marginLeft:'4px' }}>{request.title} </Box>

          <CircleIcon sx= {{width:'16px', height:'16px', marginRight:'4px', color:'#F44336'}}/>
