...
class collectionService {
  ...
  delete = createAsyncThunk('collectionService/delete', async (collection: collectionItem, thunkAPI) => {
    const state = thunkAPI.getState() as RootState;
    const workspace = selectWorkspaceById(state, collection.workspaceId);
    const cloned = JSON.parse(JSON.stringify(workspace));
  ...
  update = createAsyncThunk('collectionService/update', async (collection: collectionItem, thunkAPI) => {
